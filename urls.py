...
urlpatterns = [
    ...
    url(r'^password_reset/', password_reset, {'template_name': 'lostpass.html'}, name='password_reset'),
    # override the template with ours
    url(r'^password_reset_done/', password_reset_done, {'template_name': 'resetdone.html'}, name='password_reset_done'),
    # override the template with ours
    url(r'^password_reset_confirm/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$',
        password_reset_confirm, {'template_name': 'resetconfirm.html', 'post_reset_redirect': '/'},
