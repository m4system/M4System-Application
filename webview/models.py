...
class UserProfile(models.Model):
    ...
    * We need to add a signal handler for the automated creation of the profile when a new user is created
    ...
    loggedin = models.BooleanField('Currently logged-in.', default=False, help_text='Is this user currentely logged-in ?')
    notifemail = models.EmailField('Notification Email', blank=True, null=True, default=None, help_text='Fill this field to receive email alerts')
    notifcallback = models.URLField('Notification URL', max_length=1024, null=True, blank=True, help_text='Fill this field to receive POST callbacks to the specified URL')
    prefs = models.CharField('Settings', max_length=4096, blank=True, help_text='JSON Object containing all of the UI\'s settings.')
...
class Widgets(models.Model):
    ...
    hostcheck = models.ForeignKey(HostChecks, on_delete=models.SET_NULL, null=True, blank=True, help_text='Which check does this widget refers to ?')
    host = models.ForeignKey(Hosts, on_delete=models.SET_NULL, null=True, blank=True, help_text='Which host does this widget refers to ?')
    unit = models.CharField('Unit', max_length=4, default='A', blank=True, help_text='Unit used for the check in the widget, i.e. A')
    template = models.CharField('Template', max_length=32, choices=TEMPLATE_CHOICES, default='snmpgetint', help_text='The template to use to display the widget.')
    ...
    def renderWidget(self, user = None):
        ...
        if self.hostcheck.checktype == 'snmpgetbool' or self.hostcheck.checktype == 'execbool':
            ...
            hostcheckmd['lastfalse'] = getMetadata(self.host.name + ':' + self.hostcheck.name + '::lastfalse', 'No Data')
        ...
        c = {'host': self.host, 'hostcheck': self.hostcheck, 'name': self.name, 'hostcheckmd': hostcheckmd, 'value': value, 'unit': self.unit, 'verbosename': self.verbosename, 'color': color, 'user': user, 'notifs': notifs}
...
class UIMsg(models.Model):
    ...
    """
    LEVEL_CHOICES = (('20', 'info'), ('10', 'debug'), ('30', 'warning'), ('25', 'success'), ('40', 'error'), ('99', 'problem'))
    level = models.CharField('Level', max_length=16, choices=LEVEL_CHOICES, default='debug', help_text='The level of the message')
