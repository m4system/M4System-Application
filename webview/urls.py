...
urlpatterns = [
    ...
    url(r'^data/(?P<host>[A-Za-z0-9\-\_]+)/(?P<check>[A-Za-z0-9\_\-]+)/(?P<checktype>[a-z]+)/$', getCheck, name='getCheck'),
    url(r'^graph/(?P<host>[A-Za-z0-9\-\_]+)/(?P<check>[A-Za-z0-9\_\-]+)(/|/(?P<qty>[0-9]+)/)$', getGraphData, name='getGraphData'),
    ...
    url(r'^widget/(?P<host>[A-Za-z0-9\-\_]+)/(?P<check>[A-Za-z0-9\_\-]+)/(?P<name>[A-Za-z0-9\-\_]+)/$', getWidget, name='getWidget'),
    ...
    url(r'^notifications/m4-(?P<host>[A-Za-z0-9\-\_]+)--(?P<check>[A-Za-z0-9\_\-]+)-notifs/(?P<status>[a-z]+)/$', setNotifs, name='setNotifs'),
]
...
if settings.DEBUG or TESTING:
    ...
    urlpatterns += [url(r'^__debug__/', include(debug_toolbar.urls)), url(r'^public/(?P<path>.*)$', views.serve)]  # for the debug toolbar
