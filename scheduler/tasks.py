...
def snmpgetint(self, host, check):
    ...
    try:
        ...
        setMetadata(hostname.name + ':' + checkname.name + '::nberror', int(getMetadata(hostname.name + ':' + checkname.name + '::nberror', 0)) + 1)
...
def snmpgetstr(self, host, check):
    ...
    try:
        ...
        setMetadata(host.name + ':' + check.name + '::nberror', int(getMetadata(host.name + ':' + check.name + '::nberror', 0)) + 1)
        # Log the error to the database.
...
def Heartbeat(self):
    ...
    emails = []
    emails.append(('[YUL62-BMS] Heartbeat', 'This email confirms YUL62 BMS is online and operational.', 'm4@m4system.com', ['m4@m4system.com']))
